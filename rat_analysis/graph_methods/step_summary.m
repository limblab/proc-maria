...
%this. also TODO: graph the joint angles while all of this is going on.
%just use swing phase, and do the overlay+avg of hip, knee, ankle angles
sw_idx = [sort(pk_positions.f_locs(1:11)) sort(pk_positions.b_locs(1:11))];
...
if usr_in == 'y'
    ...
    filename = 'summary_steps_test.mat';
    ...
    if exist([savepath filename])
        ...
        if contains(filedate, filedate_temp)
            ...
            for j=1:length(file_idx)
                ...
                if filenum{file_idx(j)} == filenum_temp
                    ...
                    '(y to overwrite/a to append/n to quit without saving) '], 's');
                    break
                end
            end
        end
        ...
        if cont_write == 'a'
            %TODO: change this so it's overwriting, only in the case where
            %it has that trial saved already, and otherwise will be
            %appending
            %TODO: change way of saving fig file so if that file exists you
            %add to it
            for i=1:length(vars)
            ...
            savefig([figpath filedate_temp '_' num2str(filenum_temp)]);
            %TODO: save the angle fig too, here and below
        elseif cont_write == 'y'

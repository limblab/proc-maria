...
%segment
figure(70); hold on; 
prange = [20:checkrange(1)-1 checkrange];
plot(fdata.mag_vel(prange), 'linewidth', 2) %magnitude of accel
plot(fdata.mag_acc(prange), 'linewidth', 2) %magnitude of accel
plot(2:length(prange), diff(fdata.mag_vel(prange)), 'linewidth', 2) %accel including negatives
plot(1:length(prange), zeros(1, length(prange)), 'color', 'k') %x axis
ax = gca; 
line([locs(p)-prange(1)+1 locs(p)-prange(1)+1], ax.YLim, 'LineStyle','--', 'color', 'k')
